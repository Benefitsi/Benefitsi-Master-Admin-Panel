...
2. For a first setup with no existing callers, configure a random secret of at least 32 characters as `BENEFITSI_WEB_REVALIDATION_SECRET` in both matching server environments. For an existing installation, preserve both projects' current credentials and use the additive `BENEFITSI_PUBLISHER_REVALIDATION_SECRET` mapping below instead. Never use a `NEXT_PUBLIC_` variable, application login password or service-role key.
...
For an additive credential transition, set the same new server-only `BENEFITSI_PUBLISHER_REVALIDATION_SECRET` in the matching Web and Admin projects. Preserve every existing `BENEFITSI_WEB_REVALIDATION_SECRET` and `BENEFITSI_ADMIN_REVALIDATION_SECRET` value in both projects, including write-only Production credentials; do not copy the new value over a legacy key. Both Admin senders prefer the optional Publisher key. When it is absent or whitespace-only, microsites retain their WEB-only fallback, and City retains ADMIN then WEB. A nonblank Publisher value must contain at least 32 printable ASCII characters without internal whitespace; malformed values return `not_configured` before lookup or delivery instead of falling back. The trimmed secret is sent only in the server-to-server Authorization header; it is never a client property, URL parameter or `NEXT_PUBLIC_` variable.

Activate and verify the new Web receiver before the new Admin producer. For rollback, restore the previous Admin deployment first; the preserved legacy keys allow its unchanged sender to keep working. The optional Publisher key does not alter the exact endpoint validation, redirect refusal or Preview protection guards. A staging-built artifact must not be promoted with production data expectations: build the production target with production environment values.
