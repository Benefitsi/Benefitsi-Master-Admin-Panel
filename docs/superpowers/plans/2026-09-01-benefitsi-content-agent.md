...
- [x] **Step 1: Write failing tests** for task validation, prompt data boundaries, JSON parsing, maximum length, and rejection of empty/malformed output.
- [x] **Step 2: Run `node --import tsx --test tests/content-agent.test.mjs` and confirm the tests fail because the module is missing.**
- [x] **Step 3: Implement the smallest pure prompt/parser module with no network or Supabase dependency.**
- [x] **Step 4: Re-run the focused test and then the complete Admin Panel test suite.**
- [x] **Step 5: Commit as `feat: add Benefitsi content agent contract`.**
...
- [x] **Step 2: Run the bridge regression tests and confirm the new assertions fail.**
- [x] **Step 3: Implement the guarded bridge action and profile files; keep existing Ben/city actions unchanged.**
- [x] **Step 4: Run Python syntax checks and the existing MCP/bridge tests.**
- [x] **Step 5: Sync only the non-secret profile and bridge source to the M1, create a timestamped backup, and verify file permissions; do not copy credentials.**
- [x] **Step 6: Run an authenticated read-only bridge status check and a bounded content-agent smoke test only if the MiniMax credential is available; do not write live content.**
- [x] **Step 7: Commit tracked source/profile changes as `feat: add guarded Benefitsi content Hermes profile`.**
...
- [x] **Step 1: Write failing action tests that assert the request body uses `benefitsi-content` and the expected task for partner/deal/staff copy, and that bridge errors remain draft-only.**
- [x] **Step 2: Run the focused tests and confirm they fail against the current `profile: "ben"` request body.**
- [x] **Step 3: Refactor the server actions to use a shared bounded bridge helper and the pure content contract.**
- [x] **Step 4: Update user-facing labels/messages from “Ben” to “Content-Agent” where the action is now delegated, while preserving Ben labels for true Ben review actions.**
- [x] **Step 5: Run focused action tests, then all tests and lint.**
- [x] **Step 6: Commit as `feat: route admin copy drafts through content agent`.**
...
- [x] **Step 1: Add a failing UI/source regression test for Content-Agent labels and draft-only insertion behavior.**
- [x] **Step 2: Run the focused test and confirm the current Ben labels fail the intended assertions.**
- [x] **Step 3: Update the suggestion button/status copy and wire any missing staff-copy field to the existing action.**
- [x] **Step 4: Run the focused test and full test/lint suite.**
- [x] **Step 5: Commit as `feat: expose content agent copy suggestions in partner admin`.**
...
- [x] **Step 1: Run `npm test`, `npm run lint`, and `npm run build` from the isolated worktree.**
- [x] **Step 2: Verify M1 profile existence, MiniMax provider, owner-only credential mode `0600`, no scheduler, and bridge status without printing secrets.**
