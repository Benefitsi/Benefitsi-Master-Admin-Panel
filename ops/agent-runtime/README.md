...
Vor Production wird die Migration auf Stage angewendet. Danach wird `supabase/tests/benefitsi_agent_runtime_snapshots_acl.test.sql` über den freigegebenen Stage-Datenbankkanal ausgeführt. Die Datei beginnt mit `begin` und endet mit `rollback`; sie prüft ACL, RLS, RPC-Validierung, Größen- und Arraylimits sowie monotone Zeitstempel. Keine Datenbank-URL oder Zugangsdaten in Befehle, Logs oder Dateien dieses Ordners schreiben. Erst nach erfolgreichem Stage-Test und Review wird dieselbe Migration auf Production angewendet.
