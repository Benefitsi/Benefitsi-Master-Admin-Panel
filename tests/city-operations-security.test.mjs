...
const genericPublishMigrationPath = new URL(
  "supabase/migrations/20260729173000_city_content_generic_publish.sql",
  webRoot,
)
const communityTrackingMigrationPath = new URL(
  "supabase/migrations/20260729210000_city_community_tracking.sql",
  webRoot,
)
const newsletterEditorialMigrationPath = new URL(
  "supabase/migrations/20260729230000_city_newsletter_editions.sql",
  webRoot,
)
...
test("central event editor persists schedules behind the event review gate", async () => {
  const [actions, definitions, page] = await Promise.all([
    readFile(
      new URL(
        "../app/city-pages/[citySlug]/content/[contentType]/[contentId]/actions.ts",
        import.meta.url,
...
test("central benefit editor uses the canonical welcome-deal label", async () => {
  ...
  assert.doesNotMatch(definitions, /value: "welcome", label: "Willkommen"/)
})
...
test("central place editor validates structured hours and visit facts behind review", async () => {
  ...
  assert.match(actions, /place_admission_validation/)
  assert.match(actions, /place_contact_validation/)
...
test("newsletter editorial actions authenticate, use gated RPCs and expose no send action", async () => {
  const [migration, actions, page] = await Promise.all([
    readFile(newsletterEditorialMigrationPath, "utf8"),
    readFile(
