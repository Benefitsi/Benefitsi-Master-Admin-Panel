...
const migrationUrl = new URL(
  "../../../benefitsi-web/supabase/migrations/20260728170000_booking_marketplace_core.sql",
  import.meta.url,
)
...
test("community moderation no longer invents a two-hour event end", async () => {
  const source = await readFile(
    new URL(
      "../../../benefitsi-web/src/app/api/admin/city-pages/[citySlug]/community/[submissionId]/route.ts",
      import.meta.url,
    ),
    "utf8",
  )
...
test("operations migration makes cancellation and refund attachment idempotent and service-only", async () => {
  const sql = await readFile(
    new URL(
      "../../../benefitsi-web/supabase/migrations/20260728190000_booking_operations.sql",
      import.meta.url,
    ),
    "utf8",
  )
...
test("public checkout reaches Stripe only through the server-to-server shared-secret proxy", async () => {
  ...
  const proxy = await readFile(
    new URL(
      "../../../benefitsi-web/src/app/api/city/[citySlug]/bookings/checkout/route.ts",
      import.meta.url,
    ),
    "utf8",
  )
...
test("public offer loader excludes test providers and exposes only active enabled providers", async () => {
  const source = await readFile(
    new URL(
      "../../../benefitsi-web/src/lib/bookings/public-bookings.ts",
      import.meta.url,
    ),
    "utf8",
  )
...
test("Annweiler booking pages keep one canonical offer path and SEO entry", async () => {
  const detail = await readFile(
    new URL(
      "../../../benefitsi-web/src/app/stadt/[citySlug]/buchen/[offerSlug]/page.tsx",
      import.meta.url,
    ),
    "utf8",
  )
  const sitemap = await readFile(
    new URL("../../../benefitsi-web/src/app/sitemap.ts", import.meta.url),
    "utf8",
  )
