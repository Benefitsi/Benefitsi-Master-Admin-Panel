...
test("PDF extraction sends original bytes as a typed attachment to Gemini and returns reviewed fields", async () => {
  const api = provider()
  const file = pdfFile()
  const result = await extractMenuFromFiles([file], { apiKey: "test-key", model: "models/test-flash", fetch: api.fetch })
  assert.deepEqual(result, draft())
  assert.equal(api.requests.length, 1)
  const request = api.requests[0]
  assert.equal(request.url, "https://generativelanguage.googleapis.com/v1beta/models/test-flash:generateContent")
  assert.equal(request.method, "POST")
  assert.equal(request.headers["x-goog-api-key"], "test-key")
  assert.equal(request.cache, "no-store")
  assert.ok(request.signal instanceof AbortSignal)
  assert.equal(request.body.tools, undefined)
  const attachments = request.body.contents[0].parts.filter((part) => part.inlineData)
  assert.equal(attachments.length, 1)
  assert.equal(attachments[0].inlineData.mimeType, "application/pdf")
  assert.deepEqual(Buffer.from(attachments[0].inlineData.data, "base64"), Buffer.from(await file.arrayBuffer()))
...
test("JPEG, PNG and WebP photos retain their order and actual MIME types", async () => {
  ...
  const api = provider()
  await extractMenuFromFiles(files, { apiKey: "test-key", fetch: api.fetch })
  const attachments = api.requests[0].body.contents[0].parts.filter((part) => part.inlineData)
  assert.deepEqual(attachments.map((part) => part.inlineData.mimeType), ["image/jpeg", "image/png", "image/webp"])
  for (let index = 0; index < files.length; index++) {
    assert.deepEqual(Buffer.from(attachments[index].inlineData.data, "base64"), Buffer.from(await files[index].arrayBuffer()))
  }
})
...
test("extraction preserves uncertain source data without fabricating prices, currency or allergens", async () => {
  ...
  assert.deepEqual(await extractMenuFromFiles([pdfFile()], { apiKey: "key", fetch: api.fetch }), value)
...
test("extraction preserves CHF source prices but review refuses unsupported currency without conversion", async () => {
  ...
  const api = provider(value)
  const extracted = await extractMenuFromFiles([pdfFile()], { apiKey: "key", fetch: api.fetch })
...
test("size limits reject overlarge menus instead of silently dropping categories or items", async () => {
  ...
  for (const value of [manyCategories, manyItems]) {
    ...
    const api = provider(value)
    await assert.rejects(extractMenuFromFiles([pdfFile()], { apiKey: "key", fetch: api.fetch }), /aufteilen|Teil/)
  }
  const maximum = draft()
...
test("file validation rejects unsupported, spoofed and mixed files before making any provider call", async (t) => {
  ...
  for (const [name, files] of cases) await t.test(name, async () => {
    let requests = 0
    await assert.rejects(extractMenuFromFiles(files, { apiKey: "key", fetch: async () => { requests++; throw new Error("must not call") } }))
...
test("the 4 MiB upload boundary accepts its limit and rejects one extra byte across multiple files", async () => {
  ...
  await extractMenuFromFiles([pdfFile(maximumPdf)], { apiKey: "key", fetch: atLimit.fetch })
  ...
  await assert.rejects(extractMenuFromFiles(photos, { apiKey: "key", fetch: tooLarge.fetch }), /höchstens 4 MiB/)
...
test("missing provider configuration never sends a menu", async () => {
  let requests = 0
  await assert.rejects(extractMenuFromFiles([pdfFile()], { apiKey: " ", fetch: async () => { requests++; throw new Error("must not call") } }), /eingerichtet|konfiguriert/)
  assert.equal(requests, 0)
})

test("incomplete, blocked, malformed and truncated provider output is never returned as an importable draft", async (t) => {
  const cases = [
    ["provider truncation", { candidates: [{ finishReason: "MAX_TOKENS", content: { parts: [{ text: JSON.stringify(draft()) }] } }] }],
    ["provider safety stop", { candidates: [{ finishReason: "SAFETY", content: { parts: [{ text: JSON.stringify(draft()) }] } }] }],
    ["provider blocks prompt", { promptFeedback: { blockReason: "SAFETY" }, candidates: [] }],
    ["no candidates", { candidates: [] }],
    ["missing finish reason", { candidates: [{ content: { parts: [{ text: JSON.stringify(draft()) }] } }] }],
    ["invalid JSON", { candidates: [{ finishReason: "STOP", content: { parts: [{ text: "{broken" }] } }] }],
    ["incomplete extraction", { candidates: [{ finishReason: "STOP", content: { parts: [{ text: JSON.stringify({ ...draft(), complete: false }) }] } }] }],
    ["empty content", { candidates: [{ finishReason: "STOP", content: { parts: [] } }] }],
  ]
  for (const [name, response] of cases) await t.test(name, async () => {
    await assert.rejects(extractMenuFromFiles([pdfFile()], { apiKey: "key", fetch: async () => Response.json(response) }))
  })
})
...
test("provider transport errors are actionable German messages without echoing upstream secrets", async (t) => {
  for (const status of [400, 401, 403, 429, 500, 503]) await t.test(String(status), async () => {
    await assert.rejects(extractMenuFromFiles([pdfFile()], { apiKey: "private-key", fetch: async () => Response.json({ error: { message: "private-key/provider-internal-detail" } }, { status }) }), (error) => {
      assert.doesNotMatch(error.message, /private-key|provider-internal-detail/)
      assert.match(error.message, /KI|erneut|Limit|Zugang/)
      return true
    })
  })
  await assert.rejects(extractMenuFromFiles([pdfFile()], { apiKey: "private-key", fetch: async () => { throw new TypeError("private-key upstream network error") } }), /Verbindung|erreichbar|erneut/)
})
