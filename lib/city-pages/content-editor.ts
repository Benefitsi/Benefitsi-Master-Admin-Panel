...
import {
  ...
  loadFieldControls,
  type CityEditorEntityType,
  type CityFieldControl,
...
export type EditorField = {
  ...
  relationType?: "places" | "events" | "routes" | "guides"
}
...
const imageFields: EditorField[] = [
  {
    name: "image_url",
    label: "Legacy-Fallback-Bild-URL",
    kind: "url",
    helper: "Primäre Hero-, Card- und Galerie-Bilder bitte in der Media Library zuweisen.",
    placeholder: "https://…",
  },
  {
...
const seoFields: EditorField[] = [
  {
    name: "seo_title",
    label: "SEO-Titel",
    kind: "text",
    maxLength: 180,
    helper: "Optionaler manueller Titel für Suchergebnis, Open Graph und Detailseite.",
  },
  {
    ...
    rows: 3,
    maxLength: 500,
    helper: "Optionaler manueller Beschreibungstext. Leer = automatische Ableitung.",
  },
]
...
> = {
  events: {
    ...
    fields: [
      ...
      },
      {
        ...
        kind: "text",
        maxLength: 500,
        helper: "Optionaler Kurztext. Wenn strukturierte pricing-Tarife vorhanden sind, werden diese öffentlich verwendet.",
  ...
  routes: {
    ...
    fields: [
      ...
      {
        name: "highest_point_meters",
        label: "Höchster Punkt in Metern",
        kind: "number",
        min: 0,
        max: 10000,
        step: 1,
      ...
      {
        name: "lowest_point_meters",
        label: "Niedrigster Punkt in Metern",
        kind: "number",
        min: 0,
        max: 10000,
        step: 1,
      ...
      {
        name: "related_place_ids",
        label: "Verknüpfte Orte",
        kind: "relations",
        relationType: "places",
        helper: "Nur Orte aus derselben Stadt werden angeboten.",
      },
      {
        name: "related_route_ids",
        label: "Verknüpfte Routen",
        kind: "relations",
        relationType: "routes",
      },
      {
        name: "related_event_ids",
        label: "Verknüpfte Veranstaltungen",
        kind: "relations",
        relationType: "events",
      },
      {
        name: "related_guide_ids",
        label: "Verknüpfte Guides",
        kind: "relations",
        relationType: "guides",
      },
      ...seoFields,
...
export type EditorRelationOptions = {
  places: EditorOption[]
  events: EditorOption[]
  routes: EditorOption[]
  guides: EditorOption[]
...
export type CityEditorMediaAssignment = {
  id: string
  mediaAssetId: string
  role: string
  publicUrl: string
...
export type CityEditorAuditEntry = {
  id: string
  fieldPath: string
  action: string
  beforeValue: unknown
...
async function loadRelationOptions(
  ...
  cityId: string,
): Promise<EditorRelationOptions> {
  type CityRelationTable = "city_places" | "city_events" | "city_routes" | "city_guides"
  const definitions = [
    { key: "places" as const, table: "city_places", labelField: "name" },
    { key: "events" as const, table: "city_events", labelField: "title" },
    { key: "routes" as const, table: "city_routes", labelField: "title" },
    { key: "guides" as const, table: "city_guides", labelField: "title" },
  ]
  ...
  const results = await Promise.all(
    definitions.map(async (definition) => ({
      definition,
      result: await admin
        .from(definition.table as CityRelationTable)
        .select(`id,${definition.labelField}`)
        .eq("city_id", cityId)
        .order(definition.labelField),
    })),
  )

  const options: EditorRelationOptions = {
    places: [],
    events: [],
    routes: [],
    guides: [],
  ...
  for (const { definition, result } of results) {
    ...
    options[definition.key] = rows.flatMap((row) => {
      const id = typeof row.id === "string" ? row.id : ""
      const labelValue = row[definition.labelField]
...
async function loadMediaAssignments(
  admin: ReturnType<typeof createAdminClient>,
  cityId: string,
  entityType: CityEditorEntityType,
  entityId: string,
): Promise<CityEditorMediaAssignment[]> {
  const assignmentResult = await admin
    .from("city_media_assignments")
    .select("id,media_asset_id,role,is_primary,manual_lock,sort_order")
    .eq("city_id", cityId)
  ...
  const assetIds = assignmentResult.data.map((row) => row.media_asset_id)
  const assetResult = await admin
    .from("city_media_assets")
    .select("id,public_url,title,alt_text,status")
    .in("id", assetIds)

  if (assetResult.error) return []
  const assets = new Map(
    (assetResult.data ?? []).map((asset) => [asset.id, asset]),
  )

  return assignmentResult.data.flatMap((assignment) => {
    const asset = assets.get(assignment.media_asset_id)
    if (!asset) return []
    return [{
      id: assignment.id,
      mediaAssetId: assignment.media_asset_id,
      ...
      manualLock: assignment.manual_lock,
...
export async function loadCityContentEditor(
  ...
  if (contentId === "new") {
    const relationOptions = await loadRelationOptions(admin, cityResult.data.id)
    return {
      city: cityResult.data as { id: string; name: string; slug: string },
      record: null,
      definition,
      entityType: fieldControlEntityType(contentType),
      fieldControls: {} as Record<string, CityFieldControl>,
      relationOptions,
      mediaAssignments: [] as CityEditorMediaAssignment[],
      audits: [] as CityEditorAuditEntry[],
    }
  }
  ...
  const [fieldControls, relationOptions, mediaAssignments, auditResult] = await Promise.all([
    loadFieldControls(admin, cityResult.data.id, entityType, contentId),
    loadRelationOptions(admin, cityResult.data.id),
    loadMediaAssignments(admin, cityResult.data.id, entityType, contentId),
    admin
      .from("city_editor_audit")
      .select("id,field_path,action,before_value,after_value,actor_profile,created_at")
      .eq("city_id", cityResult.data.id)
      .eq("entity_type", entityType)
  ...
  const audits: CityEditorAuditEntry[] = auditResult.error
    ? []
    : ((auditResult.data ?? []) as Array<Record<string, unknown>>).map((row) => ({
        id: String(row.id),
        fieldPath: String(row.field_path),
        action: String(row.action),
        beforeValue: row.before_value,
        afterValue: row.after_value,
        actorProfile: typeof row.actor_profile === "string" ? row.actor_profile : null,
